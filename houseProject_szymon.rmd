...
# TODO stopifnot(!is.element("ExterCond", varNumNames))
...
varNumStd <- predict(preNum, varNum) # TODO What is going on here?
...
varTrain <- varFull[0:nrow(train), ]
```

## A multiple linear regression model
First we log transformation of SalePrice to remove right skewness of the data.
```{r fig8, fig.align = "center", fig.cap = "Distributions of log SalePrice", message=FALSE, warning=FALSE, echo=FALSE}
set.seed(2019)

varTrain$logSalePrice <- log(train$SalePrice)
ggplot(varTrain, aes(x=logSalePrice)) + geom_histogram(fill="steelblue2")
```

Consequently we will take a look at 11 most influential features chosen by the decision tree. 
Out of them, 3 groups correlate pairwise more than 0.7.
They are:
  1. Numerical variables: TotalBsmtSF, 1stFlrSF, 2ndFlrSF
  2. Ordinal variables: OverallQual ~ ExterQual
  3. Numerical variables: GarageCars ~ GarageArea


\textcolor{red}{TODO} Should we care that OverallQual ~ ExterQual are order features?
We will try to choose and possibly engineer new variables from each group to minimize complexity of the final model.

### 1stFlrSF ~ TotalBsmtSF ~ 2ndFlrSF
```{r message=FALSE, warning=FALSE, echo=FALSE}
varTrain$hasBsmt <- varTrain$TotalBsmtSF > -3
ggplot(varTrain, aes(x = `1stFlrSF`, y = logSalePrice, colour = hasBsmt)) + geom_point() + geom_smooth(method = lm)
```
```{r message=FALSE, warning=FALSE, echo=FALSE}
varTrain$hasSndFloor <- varTrain$`2ndFlrSF` > 0
ggplot(varTrain, aes(x = `1stFlrSF`, y = logSalePrice, colour = hasSndFloor)) + geom_point() + geom_smooth(method = lm)
```
```{r message=FALSE, warning=FALSE, echo=FALSE}
model<-lm(logSalePrice ~ 1 + `1stFlrSF` + hasSndFloor + `1stFlrSF`:hasSndFloor, data = varTrain)
# anova(model)
```

```{r message=FALSE, warning=FALSE, echo=FALSE}
model<-lm(logSalePrice ~ 1 + `1stFlrSF` + hasBsmt + hasSndFloor + `1stFlrSF`:hasBsmt + `1stFlrSF`:hasSndFloor, data = varTrain)
# anova(model)
```


\textcolor{red}{TODO: } Ok, I don't know how to analyze the results from this one. The variables are interpreted as important, but later they're dropped by AIC in final model.


```{r message=FALSE, warning=FALSE, echo=FALSE}
require(gridExtra)

plot1 <- ggplot(data = varTrain, aes(x = varTrain$`1stFlrSF`, y = logSalePrice)) +
geom_point(alpha = .125) + geom_smooth(method = "lm")

plot2 <- ggplot(data = varTrain[varTrain$TotalBsmtSF > -3,], aes(x = varTrain$TotalBsmtSF[varTrain$TotalBsmtSF > -3], y = logSalePrice)) +
geom_point(alpha = .125) + geom_smooth(method = "lm") + xlab("TotalBsmtSF")

plot3 <- ggplot(data = varTrain[varTrain$`2ndFlrSF` > 0,], aes(x = varTrain$`2ndFlrSF`[varTrain$`2ndFlrSF` > 0], y = logSalePrice)) +
geom_point(alpha = .125) + geom_smooth(method = "lm") + xlab("2ndFlrSF")
...
```{r message=FALSE, warning=FALSE, echo=FALSE}
modelA <- lm(logSalePrice ~ 1 + `1stFlrSF`, data = varTrain)
modelB <- lm(logSalePrice ~ 1 + `2ndFlrSF`, data = varTrain)

model <- lm(logSalePrice ~ 1 + `1stFlrSF` + TotalBsmtSF + `2ndFlrSF`, data = varTrain)
modelWithNewFeatures <- lm(logSalePrice ~ 1 + `1stFlrSF` + hasBsmt + hasSndFloor, data = varTrain)
...
AIC(model)
BIC(model)
AIC(modelWithNewFeatures)
BIC(modelWithNewFeatures)
```

```{r message=FALSE, warning=FALSE, echo=FALSE}
par(mfrow=c(2,2))
plot(modelWithNewFeatures)
```


### OverallQual ~ ExterQual
For these variables we can notice that ExterQual and OverallQual correllate, but the latter has higher granularity of its domain, therefore performs better when predicting SalePrice.


```{r message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = varTrain, aes(x = varTrain$OverallQual, y = logSalePrice)) + geom_point(alpha = .125) + geom_smooth(method = "lm")
ggplot(data = varTrain, aes(x = varTrain$ExterQual, y = logSalePrice)) + geom_point(alpha = .125) + geom_smooth(method = "lm")
```

```{r message=FALSE, warning=FALSE, echo=FALSE}
modelA <- lm(logSalePrice ~ 1 + OverallQual, data = varTrain)
modelB <- lm(logSalePrice ~ 1 + ExterQual, data = varTrain)
model <- lm(logSalePrice ~ 1 + OverallQual + ExterQual, data = varTrain)

drop1(model, test="F")

paste("OverallQual + ExterQual: ", AIC(model))
paste("OverallQual: ", AIC(modelA))
paste("ExterQual: ", AIC(modelB))
```

### GarageCars ~ GarageArea
```{r message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = varTrain, aes(x = varTrain$GarageCars, y = logSalePrice)) + geom_point(alpha = .125) + geom_smooth(method = "lm")
ggplot(data = varTrain, aes(x = varTrain$GarageArea, y = logSalePrice)) + geom_point(alpha = .125) + geom_smooth(method = "lm")
```
Contrary logic applies here, where GarageCars turns out to be better predictor than GarageArea. It's probably because prospective buyers are interested in number of cars that can fit in the garage rather than in correlated GarageArea that may not be as usable.

```{r message=FALSE, warning=FALSE, echo=FALSE}
modelA <- lm(logSalePrice ~ 1 + GarageCars, data = varTrain)
modelB <- lm(logSalePrice ~ 1 + GarageArea, data = varTrain)
model <- lm(logSalePrice ~ 1 + GarageCars + GarageArea, data = varTrain)

paste("GarageCars + GarageArea: ", AIC(model))
paste("GarageCars: ", AIC(modelA))
paste("GarageArea: ", AIC(modelB))
```
### Nominal features
Now let's analyze nominal features using TukeyHSD - TODO: How should we analyze these resuls?
```{r message=FALSE, warning=FALSE, echo=FALSE}
summary(fm1 <- aov(SalePrice ~ Neighborhood, data = train))
tuk <- TukeyHSD(fm1, "Neighborhood", ordered = TRUE)
plot(tuk)
```

### Final models
We compare 5 models
  1. Using only 3 most influential features: Neighborhood + GrLivArea + OverallQual
  2. All 11 influential features
  3. Using 11 features without removed insignificant correlated features
  4. Same as above but with hasSndFloor and hasBsmt
  5. Using all features

```{r message=FALSE, warning=FALSE, echo=FALSE}

# eg. create_model('^Land|^Neighborhood')
create_model <- function(nominal_fs_grep) {
  featureNames <- grep(nominal_fs_grep, names(varTrain), value=TRUE)
  smallFeatures <- varTrain[, union(featureNames, c('logSalePrice'))]
  model <- lm(logSalePrice ~ ., data=smallFeatures)
  return(model)
}

lm.small <- create_model('^Neighborhood|^GrLivArea|^OverallQual')
lm.big <- create_model('^Neighborhood|^GrLivArea|^OverallQual|^1stFlrSF|^MSSubClass|^GarageCars|^2ndFlrSF|^GarageArea|^TotalBsmtSF|^BsmtFinType1|^ExterQual')
lm.trimmed <- create_model('^Neighborhood|^GrLivArea|^OverallQual|^1stFlrSF|^MSSubClass|^GarageCars|^BsmtFinType1')
tm.trimeed_with_floors <- create_model('^Neighborhood|^GrLivArea|^OverallQual|^1stFlrSF|^MSSubClass|^GarageCars|^BsmtFinType1|^hasSndFloor|^hasBsmt')
lm.all <- create_model('*')
```

We can see that AIC scores for respective models remain relatively low with 3rd model compared to 2nd and 4th.  

```{r message=FALSE, warning=FALSE, echo=FALSE}
# leverageOneObservations
# varTrain <- varTrain[-leverageOneObservations, ]
# logSalePrice <- logSalePrice[-leverageOneObservations]
# fullFeature_lm <- glm(logSalePrice ~ ., data=varTrain)

AIC(lm.small)
AIC(lm.big)
AIC(lm.trimmed)
AIC(tm.trimeed_with_floors)
AIC(lm.all)
```

Plotting diagnostic graphs for trimmed model we can notice that observations 199 and 596 remain outliers. 
...
# TODO: Move smwh removal of leverage 1
# leverageOneObservations <- which(hatvalues(fullFeature_lm) %in% c(1))
```


TODO
``{r}
lm.resid <- residuals(fullFeature_lm)
sorted_residuals <- sort(abs(lm.resid), decreasing=TRUE)

par(mfrow=c(1, 2))
plot(sort(lm.resid, decreasing=TRUE))
plot(sort(abs(lm.resid), decreasing=TRUE))
``

TODO
``{r}
qq_outliers <- which(abs(lm.resid) > 0.3)
qq_outliers

varTrain <- varTrain[-qq_outliers, ]
logSalePrice <- logSalePrice[-qq_outliers]
...
* Outliers
  ...
  - Removing outliers in residuals distribution
* Skewness
  - Applying log
